...
freeproc(struct proc *p)
{
  ...
  p->xstate = 0;
  p->state = UNUSED;
  uvmunmap(p->kpagetable, p->kstack, 1, 1);//清除堆栈
  p->kstack = 0;
  ukm_freewalk(p->kpagetable);
}
...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  u2k_pgtblcopy(p->pagetable,p->kpagetable,0,p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    u2k_pgtblcopy(p->pagetable,p->kpagetable,p->sz,sz);
...
fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  u2k_pgtblcopy(np->pagetable,np->kpagetable,0,np->sz);
  release(&np->lock);
